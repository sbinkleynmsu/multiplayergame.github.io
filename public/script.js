...
function moveBall(speedX, speedY, directX, directY) {
  ...
  if (ballCoord.left <= boardCoord.left || ballCoord.right >= boardCoord.right) {
    ...
    if (parseInt(score1.textContent) >= 10 || parseInt(score2.textContent) >= 10) {
      let winner = parseInt(score1.textContent) >= 10 ? "Player 1" : "Player 2";
      alert(`Game Over! ${winner} wins!`);
      newGame(); // Reset scores and prepare for a new game
    }
    return;
  }
...
//   // Update game state based on data received from server
//   // You can update the DOM elements accordingly
// });

// // Sending initial state to the server when connected
// socket.on('connect', () => {
//   socket.on('initialState', (initialState) => {
//     // Update the game state with the initial state received from the server
//   });
// });
